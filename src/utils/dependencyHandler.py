...
def parseDownloadProgress(line: str) -> str | None:
    """
    Parse pip output to extract download progress percentage.
    
    Args:
        line: A line of pip output
        
    Returns:
        Progress message with percentage if found, None otherwise
    """
    # Match pip download progress patterns like "Downloading package (45.2 MB)" or percentage patterns
    # Pip shows progress like: "Downloading torch-2.9.1+cu130-cp313-cp313-win_amd64.whl (2.4 GB)"
    # followed by progress bar lines with percentages
    
    # Match percentage pattern from pip progress bar (e.g., "   45%|████...")
    percentMatch = re.search(r'(\d{1,3})%\|', line)
    if percentMatch:
        percent = percentMatch.group(1)
        return f"[DOWNLOAD_PROGRESS:{percent}]"
    
    # Match "Downloading <package>" lines
    downloadMatch = re.search(r'Downloading\s+([^\s]+)', line)
    if downloadMatch:
        package = downloadMatch.group(1).split('/')[-1]  # Get filename from URL
        # Truncate long package names
        if len(package) > 50:
            package = package[:47] + "..."
        return f"[DOWNLOAD_START:{package}]"
    
    # Match "Installing collected packages:" line
    if "Installing collected packages:" in line:
        return "[INSTALL_START]"
    
    # Match "Successfully installed" line
...
def installDependencies(extension: str = "", isNvidia: bool = True) -> Tuple[bool, str]:
    ...
    # Use --progress-bar on to ensure pip shows download progress
    cmd = f'"{pythonPath}" -I -m pip install -r "{requirementsPath}" --no-warn-script-location --no-cache --disable-pip-version-check --progress-bar on'
    ...
    try:
        ...
        for line in streamProcessOutput(process):
            # Parse and emit progress information for GUI
            progressInfo = parseDownloadProgress(line)
            if progressInfo:
                print(progressInfo)
