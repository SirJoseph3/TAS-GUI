...
def detectGPUArchitecture():
    ...
    """
    try:
        # On Windows, nvidia-smi is often *not* on PATH even when NVIDIA drivers are installed.
        # Reuse our known-path search to avoid falling back to the CPU-only requirements by mistake.
        smiCandidates = getNvsmipaths() + ["nvidia-smi"]

        for smi in smiCandidates:
            if not smi:
                continue
            if smi != "nvidia-smi" and not os.path.exists(smi):
                continue
            ...
            result = subprocess.run(
                [smi, "-L"], capture_output=True, text=True, check=False
            )
