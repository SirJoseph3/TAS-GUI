...
def modelsMap(
    ...
    # OpenVINO variants use the same ONNX weights as DirectML variants.
    if model.endswith("-openvino"):
        model = model.replace("-openvino", "-directml")
    ...
    match model:
        ...
        case "yolov9_small-directml" | "yolov9_medium-directml" | "yolov9_large-directml":
            if modelType == "onnx":
                modelName = model.replace("-directml", "")
...
def downloadModels(
    ...
    if (
        model.endswith("-tensorrt")
        or model.endswith("-directml")
        or model.endswith("-openvino")
    ):
        if "rife" in model:
