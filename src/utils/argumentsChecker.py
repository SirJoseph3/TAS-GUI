...
def createParser(outputPath):
    ...
    performanceGroup.add_argument(
        "--tile_rendering",
        action="store_true",
        help="Enable tile rendering (tiled inference). When disabled, no automatic tile sizing or OOM tile retry will be used.",
    )
    ...
    performanceGroup.add_argument(
        ...
        type=int,
        default=128,
        help="Tile size for tile rendering (only used when --tile_rendering is enabled). Minimum 128.",
    )
...
def _addInterpolationOptions(argParser):
    ...
    interpolationGroup.add_argument(
        ...
        type=str,
        default="rife4.6",
        help="Interpolation method (builtin model name or path to custom model)",
    )
    interpolationGroup.add_argument(
        "--slowmo",
        action="store_true",
    ...
    interpolationGroup.add_argument(
        "--custom_interpolate_backend",
        type=str,
        choices=["default", "cuda", "tensorrt", "directml", "ncnn"],
        default="default",
        help="Backend to use for custom interpolate models. 'default' auto-detects based on file extension.",
    )
...
def _addUpscalingOptions(argParser):
    ...
    upscaleGroup.add_argument(
        "--image_mode", action="store_true", help="Enable image mode for single image upscaling"
    )
    upscaleGroup.add_argument(
        "--upscale_factor",
        type=int,
    ...
    upscaleGroup.add_argument(
        ...
        help="Upscaling method (builtin model name or path to custom model)",
    )
    upscaleGroup.add_argument(
    ...
    upscaleGroup.add_argument(
        ...
        choices=["default", "cuda", "tensorrt", "directml", "ncnn"],
        default="default",
        help="Backend to use for custom upscale models. 'default' auto-detects based on file extension.",
    )
...
def _addDedupOptions(argParser):
    ...
    dedupGroup.add_argument(
        ...
        type=str,
        default="ssim",
        help="Deduplication method (builtin method name or path to custom model)",
    )
    dedupGroup.add_argument(
...
def _addVideoProcessingOptions(argParser):
    ...
    processingGroup.add_argument(
        ...
        help="Denoising method(s), can specify multiple for chaining (builtin model name or path to custom model)",
    )
    processingGroup.add_argument(
...
def _addSegmentationOptions(argParser):
    ...
    segmentationGroup.add_argument(
        ...
        type=str,
        default="anime",
        help="Segmentation method (builtin model name or path to custom model)",
    )
...
def _addSceneDetectionOptions(argParser):
    ...
    sceneGroup.add_argument(
        ...
        help="Scene change detection method (builtin model name or path to custom model)",
    )
    sceneGroup.add_argument(
        "--scenechange_sens",
        type=float,
...
def _addDepthOptions(argParser):
    ...
    depthGroup.add_argument(
        ...
        type=str,
        default="small_v2",
        help="Depth estimation method (builtin model name or path to custom model)",
    )
    depthGroup.add_argument(
        "--depth_quality",
        type=str,
...
def argumentsChecker(args, outputPath, parser):
    ...
    if not args.input:
        ...
        if not getattr(args, "image_mode", False):
            raise Exception(
                "Image input is not supported, use Chainner for image processing"
            )
    elif args.input.lower().endswith((".gif")):
        decode_method = getattr(args, "decode_method", "")
        if isinstance(decode_method, str) and decode_method.lower() == "nvdec":
            logging.warning(
                "GIF input detected but decode_method is set to NVDEC; defaulting to CPU decoding"
            )
            args.decode_method = "cpu"
...
def _handleDependencies(args):
    ...
    needsFfmpegDownload = (not os.path.exists(cs.FFMPEGPATH)) or (
        not os.path.exists(cs.FFPROBEPATH)
    )

    wantsNvdec = str(getattr(args, "decode_method", "cpu")).lower() == "nvdec"
    if cs.SYSTEM == "Windows" and wantsNvdec:
        ...
        for dll in required_dlls:
            if not os.path.exists(os.path.join(ffmpeg_dir, dll)):
                needsFfmpegDownload = True
                break

    if needsFfmpegDownload:
