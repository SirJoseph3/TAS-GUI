...
def generateOutputName(args, videoInput):
    ...
    encode_method = getattr(args, "encode_method", "")

    if any(proto in str(videoInput) for proto in ["https://", "http://"]):
        baseName = f"TAS-YTDLP-{random.randint(0, 1000)}"

        if getattr(args, "segment", False) or encode_method in ["prores", "prores_segment"]:
            extension = ".mov"
        elif encode_method == "png":
            extension = ""
        elif encode_method == "gif":
            extension = ".gif"
        else:
            extension = ".mp4"
...
def generateOutputPath(video, output, defaultOutputPath, args):
    ...
    if output and output.endswith(tuple(EXTENSIONS)):
        encode_method = getattr(args, "encode_method", "")
        if encode_method == "gif" and not output.lower().endswith(".gif"):
