...
class VideoDownloader:
    def __init__(
        ...
        if os.environ.get('YTDLP_LIST_QUALITIES_ONLY') == '1':
            quality_data = {
                'title': title,
                'duration': durationStr,
                'qualities': choices
            }
            print(f"[QUALITIES]{json.dumps(quality_data)}")
            exit(0)

        quality_index = os.environ.get('YTDLP_QUALITY_INDEX')
        if quality_index is not None:
            try:
                idx = int(quality_index)
                if 0 <= idx < len(choices):
                    selected = choices[idx]
                    print(f"(*) Auto-selected quality #{idx}: {selected}")
                else:
                    logging.error(f"Invalid quality index: {idx}")
                    exit(1)
            except ValueError:
                logging.error(f"Invalid quality index format: {quality_index}")
                exit(1)
        else:
            questions = [
                List(
