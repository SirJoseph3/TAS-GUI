...
class VideoProcessor:
    ...
    def _initProcessingParams(self, args):
        ...
        self.customUpscaleBackend: str = args.custom_upscale_backend
        self.customInterpolateBackend: str = args.custom_interpolate_backend
        self.customRestoreBackend: str = args.custom_restore_backend
        ...
        # Optional tile rendering controls
        self.tileRendering: bool = getattr(args, "tile_rendering", False)
        self.tileSize: int = getattr(args, "tile_size", 128)
    ...
    def process(self):
        ...
        logging.info(f"Starting progress bar with {self.totalFrames} frames, increment={increment}")
        error = None
        try:
            with self.ProgressBarLogic(
                ...
                for _ in range(self.totalFrames):
                    if frameCount == 0:
                        logging.info(f"Waiting for first frame from readBuffer...")
                    frame = self.readBuffer.read()
                    if frameCount == 0:
                        logging.info(f"First frame received: {type(frame)}")
                    if frame is None:
                        logging.info("Received None frame, breaking...")
                        break
            ...
            try:
                ...
                logging.exception("Failed to close preview server")

            try:
                self.writeBuffer.close()
            except Exception:
                logging.exception("Failed to close write buffer")
        ...
        if frameCount == 0 and self.totalFrames > 0:
            hint = ""
            if self.decodeMethod == "nvdec":
                hint = " Try --decode_method cpu."
            raise RuntimeError(
                f"No frames were processed (0/{self.totalFrames}). decode_method={self.decodeMethod}.{hint}"
            )
    ...
    def start(self):
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                logging.info("Submitting readBuffer thread")
                readFuture = executor.submit(self.readBuffer)
                logging.info("Submitting writeBuffer thread")
                writeFuture = executor.submit(self.writeBuffer)
                logging.info("Submitting process thread")
                processFuture = executor.submit(self.process)
                
                logging.info("All threads submitted, waiting for completion...")

            # Surface exceptions from worker threads and get processed frame count.
            readFuture.result()
            processedFrames = processFuture.result()
            writeFuture.result()
            ...
            error_msg = str(e)
            if "Bu model video için" in error_msg or "çok güçlü" in error_msg:
